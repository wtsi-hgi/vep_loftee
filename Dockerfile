FROM ensemblorg/ensembl-vep:release_94.5

WORKDIR /opt/vep/src

USER root
RUN apt update && apt install wget liblzma-dev libbz2-dev libcurl4-openssl-dev git -y
...
RUN make test
RUN make install
ENV PERL5LIB /opt/vep/.vep/Plugins:/opt/vep/DBD-SQLite/lib/x86_64-linux-gnu/perl/5.22.1/:$PERL5LIB
RUN echo $PERL5LIB
