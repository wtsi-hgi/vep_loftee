FROM ensemblorg/ensembl-vep:release_97.3

WORKDIR /opt/vep/src

USER root
RUN apt update && apt install wget libcurl4-openssl-dev libbz2-dev git -y
...
RUN make test
RUN make install
ENV PERL5LIB /opt/vep/.vep/Plugins:/opt/vep/DBD-SQLite/lib/x86_64-linux-gnu/perl/5.26.1/:$PERL5LIB
RUN echo $PERL5LIB
